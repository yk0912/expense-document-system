...
taxKind8 / taxKind10 は印字だけで判定してください。「内10%」「内8%」「内消費税」「内税」「うち消費税」「(内)」とある税率は included、「外10%」「外8%」「外税」「(外)」とある税率は excluded です。商品行に「内税」とあればその商品の taxKind も included です。対象額と税額が印字されている場合、税額が対象×税率/(100+税率)の切り捨て（例: 2500円の10%内税なら227）なら included、対象×税率の切り捨て（例: 2500円の10%外税なら250）なら excluded です。記載が無く判定できない場合はnullです。推測しないでください。
