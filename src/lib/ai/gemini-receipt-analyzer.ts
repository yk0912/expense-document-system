...
taxKind8 / taxKind10 は印字だけで判定してください。「内10%」「内8%」「内消費税」「内税」「うち消費税」「(内)」とある税率は included、「外10%」「外8%」「外税」「(外)」とある税率は excluded です。対象額と税額が印字されている場合、税額が対象×税率/(100+税率)の切り捨て（例: 2500円の10%内税なら227）なら included、対象×税率の切り捨て（例: 2500円の10%外税なら250）なら excluded です。同じ税率で内税と外税が混在している場合は taxKind8 / taxKind10 を null にし、各商品の item.taxKind を入れてください。記載が無く判定できない場合はnullです。推測しないでください。
...
item.taxKind は商品ごとに入れてください。商品行に「内税」とある、または下部の内税対象に含まれる商品は included、外税対象に含まれる商品は excluded です。同じ10%でも内税商品と外税商品が混在することがあります。分からない場合はnullです。
